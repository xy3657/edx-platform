...
class VisibleBlocks(models.Model):
    ...
    def bulk_get_or_create(cls, user_id, course_key, block_record_lists):
        ...
        # Eliminate hash values already cached for this user/course
        # from the list of hash values to look up in a database query.
        # The database query is necessary because there may be visible blocks
        # seen and created when grades were computed for another user that have
        # not been seen by this user.
        possibly_not_created = {brl.hash_value for brl in block_record_lists if brl.hash_value not in cached_records}
        already_created = {blocks.hashed for blocks in cls.objects.filter(hashed__in=possibly_not_created)}
        non_existent_brls = possibly_not_created - already_created
    ...
    def _initialize_cache(cls, user_id, course_key):
        ...
        prefetched = {grade.visible_blocks.hashed: grade.visible_blocks for record in grades_with_blocks}
